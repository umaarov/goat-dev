...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        @if ($isDeactivated)
            <div
                class="bg-yellow-100 dark:bg-yellow-900/50 border-l-4 border-yellow-500 dark:border-yellow-600 text-yellow-700 dark:text-yellow-300 p-4 mb-6 rounded-md"
                role="alert">
                    ...
                    <div class="ml-4 flex-1 flex flex-col">
                        <div>
                            ...
                            <div class="flex flex-wrap items-center gap-y-1">
                                <div class="flex items-center">
                                    <h1 class="text-2xl font-semibold {{ $hasBackground ? 'text-white' : 'text-gray-800 dark:text-gray-100' }}"
                                        style="font-size: 1.5rem; line-height: 2rem; font-weight: 600;">{{ $displayName }}</h1>
                                ...
                                @if($user->username === 'goat' && !$isDeactivated)
                                    <span
                                        class="ml-2 inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $hasBackground ? 'bg-green-400/25 text-green-300' : 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' }}">Moderator</span>
                                @endif
                            </div>
                            ...
                            @endif
                            <p class="{{ $hasBackground ? 'text-gray-400' : 'text-gray-500 dark:text-gray-400' }} text-xs mt-1">{{ __('messages.profile.joined_label') }}
                                <time
                                    datetime="{{ $user->created_at->toIso8601String() }}">{{ $user->created_at->format('M d, Y') }}</time>
                            </p>
                        ...
                        @if (!$isDeactivated)
                            @if (!empty($userBadges))
                                <div class="mt-3 flex flex-wrap items-center gap-2">
                                    @foreach ($userBadges as $badge)
                                        <span title="{{ $badge['name'] }} - Rank: #{{ $badge['rank'] }}"
                                              class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border {{ $badge['classes'] }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1.5 -ml-1"
                            ...
                            <div class="mt-3 flex items-center space-x-6 text-sm">
                                <div>
                                    <span
                                        class="font-semibold {{ $hasBackground ? 'text-white' : 'text-gray-800 dark:text-gray-100' }}">{{ number_format($user->posts_count) }}</span>
                                    <span
                                        class="{{ $hasBackground ? 'text-gray-300' : 'text-gray-500 dark:text-gray-400' }}">{{ trans_choice('messages.profile.posts_stat_label', $user->posts_count) }}</span>
                                </div>
                                <div>
                                    <span
                                        class="font-semibold {{ $hasBackground ? 'text-white' : 'text-gray-800 dark:text-gray-100' }}">{{ number_format($totalVotesOnUserPosts) }}</span>
                                    <span
                                        class="{{ $hasBackground ? 'text-gray-300' : 'text-gray-500 dark:text-gray-400' }}">{{ trans_choice('messages.profile.votes_collected_stat_label', $totalVotesOnUserPosts) }}</span>
                                </div>
                            </div>
        ...
        <div class="mb-4 border-b border-gray-200 dark:border-gray-700">
            <div class="flex mx-auto items-center justify-between">
                <button id="load-my-posts" data-url="{{ route('profile.posts.data', $user->username) }}"
                    ...
                    <span
                        class="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-800 dark:bg-blue-400 transition-all duration-300"
                ...
                <a href="{{ route('rating.index') }}"
                   class="px-6 py-3 font-medium text-gray-700 dark:text-gray-300 hover:text-blue-800 dark:hover:text-blue-400 focus:outline-none relative flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1.5 text-amber-500" viewBox="0 0 20 20"
                         fill="currentColor">
                        <path
                            d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
