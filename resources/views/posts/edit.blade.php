                    ...
                    <fieldset class="border border-gray-300 p-4 rounded-md">
                        ...
                        <div class="mb-3">
                            ...
                            <input id="option_one_title" type="text" name="option_one_title"
                                   ...
                                   value="{{ old('option_one_title', $post->option_one_title) }}"> {{-- Removed HTML required --}}
                            @error('option_one_title')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="mb-3">
                            <label for="option_one_image" class="block text-sm font-medium text-gray-700 mb-1">Replace
                                Image (Optional)</label>
                            @if($post->option_one_image)
                                <div class="mb-2">
                                    <p class="text-xs text-gray-500 mb-1">Current:</p>
                                    <img src="{{ asset('storage/' . $post->option_one_image) }}"
                                         alt="Current Option 1 Image" class="max-h-24 rounded border border-gray-200">
                                    <label class="mt-1 flex items-center text-xs text-gray-600">
                                        <input type="checkbox" name="remove_option_one_image" value="1"
                    ...
                    <fieldset class="border border-gray-300 p-4 rounded-md">
                        ...
                        <div class="mb-3">
                            ...
                            <input id="option_two_title" type="text" name="option_two_title"
                                   ...
                                   value="{{ old('option_two_title', $post->option_two_title) }}"> {{-- Removed HTML required --}}
                            @error('option_two_title')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="mb-3">
                            <label for="option_two_image" class="block text-sm font-medium text-gray-700 mb-1">Replace
                                Image (Optional)</label>
                            @if($post->option_two_image)
                                <div class="mb-2">
                                    <p class="text-xs text-gray-500 mb-1">Current:</p>
                                    <img src="{{ asset('storage/' . $post->option_two_image) }}"
                                         alt="Current Option 2 Image" class="max-h-24 rounded border border-gray-200">
                                    <label class="mt-1 flex items-center text-xs text-gray-600">
                                        <input type="checkbox" name="remove_option_two_image" value="1"
                                               class="mr-1 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                        Remove current image
                                    </label>
                                </div>
